...
We also performed the same transformation on the logarithm version of CO2 level. We reached to a similar conclusion based on the results of plot four to six. 
...
df %>% features(log(value), unitroot_ndiffs)
df %>% features(log(value), unitroot_nsdiffs)
df %>% features(difference(difference(log(value)),12), unitroot_kpss)
```

Because the best linear model that we developed used the multiplicative decomposition method (using logarithm of the CO2 level as the response variable). The team used the same log transformation to develop the ARIMA model. We utilized the unit root test to determine the number of differences (both seasonal and non-seasonal) to make the CO2 timeseries stationary. The unit root test results show that it requires one seasonal and one non-seasonal difference to make the timeseries stationary. This result agreed with our findings in the explanatory data analysis section. 
...
Based on the results of testing out different model combination, the SARIMA model with one seasonal and one non-seasonal differencing, three non-seasonal moving average terms and one seasonal moving average term seems to be model with the best performance (lowest AICc). 
...
```
The plot above shows the comparison between observed CO2 level and the forecast from the 1997 ARIMA model. The observed CO2 levels are closer to the upper bound of the 80% CI of the forecast. The forecast from 1997 ARIMA model shows that the expected forecast is lower than the observed CO2 values at a more significant degree than that of the linear model. However, the observed CO2 seems to be closer to the upper bound of the 80% CI of the ARIMA model. 
...
```
The observed CO2 values indicate that the CO2 level reached 420 ppm the first time in `r obs1st420`. This is earlier than the prediction from the linear model and later than the prediction from the ARIMA model. The linear model (with a tight CI band) predicted that the CO2 level would reach 420 ppm as early as in `r fx2100.1st420.lm` and the ARIMA model predicted that the CO2 level would reach 420 ppm as early as in `r fx2100.1st420.ari`. The linear model predicted that the CO2 level is expected to reach 420 in `r fx2100.avg420.lm1` and the ARIMA model predicted that the CO2 level is expected to reach 420 in `r fx2100.avg420.ari1`. The linear model provided a closer prediction to the actual date than the ARIMA model. 
...
```{r non-seasonally adjusted EDA }
co2.log.nsa.tr %>% gg_tsdisplay(log_value, plot_type = 'partial')

co2.log.nsa.tr %>% gg_tsdisplay(difference(log_value), plot_type = 'partial')

co2.log.nsa.tr %>% gg_tsdisplay(difference(difference(log_value),12), plot_type = 'partial')

co2.log.nsa.tr %>% features(log_value, unitroot_ndiffs)
co2.log.nsa.tr %>% features(log_value, unitroot_nsdiffs)
co2.log.nsa.tr %>% features(difference(difference(log_value),12), unitroot_kpss)
```
The plots above show similar trend and seasonality in the updated dataset compared to the 1997 dataset. After removing the trend and the seasonality components, the third plot suggests a model with one non-seasonal moving average, one non-seasonal autoregressive, one seasonal moving average and one seasonal autoregressive term. The rootunit test also suggested taking a non-seasonal and a seasonal differencing to make updated CO2 timeseries stationary. 

```{r non-seasonally model development}
ari.fit3 <- co2.log.nsa.tr %>% 
  model(ARIMA011011 = ARIMA(log_value ~ pdq(0,1,1) + PDQ(0,1,1)),
        ARIMA111101 = ARIMA(log_value ~ pdq(1,1,1) + PDQ(1,1,1)),
...
mul.quad.sea3 <- co2.log.nsa.tr %>% 
  model(trend_model = TSLM(log_value ~ trend() + I(trend()^2) + season()))
mul.quad.sea3 %>% report()

future.df3 <- new_data(co2.log.nsa.tr, n = 24)
lm.fx.te2 <- mul.quad.sea3 %>% forecast(new_data = future.df3)
```
The team explored a variety of model specification to find the best model to fit the updated data. Our best model includes one non-seasonal and one seasonal differencing, one non-seasonal and one seasonal moving average term (lowest AICc score).



```{r Non-seasonally adjusted model evaluation}
ari.fit3 %>% select(ARIMA011011) %>% gg_tsresiduals(lag = 36)
augment(ari.fit3) %>%
  features(.innov, ljung_box, lag=200, dof=4)

bind_rows(
  ari.fit3 %>% select(ARIMA011011) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA112101) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA111101) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te), 
  ari.fit3 %>% select(ARIMA013001) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA111201) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA111102) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  lm.fx.te2 %>% fabletools::accuracy(co2.log.nsa.te)
)

```
The residual analysis of the best performing model shows that the residuals follow a pattern similar to a white noise signal. The Ljungbox test also shows that the residuals from all developed models do not have auto correlation. 

The out-of-sample evaluation shows that all models have similar MAPE scores (ranging from 0.0178 to 0.0184). The MAPE scores show that all models are performing reasonably close to each other and there is no model that is significantly better than the others. The out-of-sample MAPE score of the updated ARIMA models is lower than the MAPE score of the ARIMA models developed in 1997, suggesting that adding more recent data improves the overall performance of the ARIMA model. Because the test MAPE score did not vary significantly among all models, we kept our model selection the same based on the AICc score.

We also developed a linear model for the non-seasonally adjusted CO2 data, the linear model has a lower MAPE than all of the ARIMA model using the updated CO2 data. 

```{r Seasonally adjusted models EDA}
co2.log.sa.tr %>% gg_tsdisplay(season_adjust, plot_type = 'partial')

co2.log.sa.tr %>% gg_tsdisplay(difference(season_adjust), plot_type = 'partial')


co2.log.sa.tr %>% features(season_adjust, unitroot_ndiffs)
co2.log.sa.tr %>% features(difference(season_adjust), unitroot_ndiffs)

co2.log.sa.tr %>% features(season_adjust, unitroot_nsdiffs)
co2.log.sa.tr %>% features(difference(difference(season_adjust)), unitroot_kpss)
```
The unit root tests suggest that taking two non-seasonal differencings will result in a stationary timeseries for the seasonally-adjusted CO2 dataset. The plots above suggest a model with one or two non-seasonal differencing, one non-seasonal moving average term, one or two non-seasonal autoregressive terms, one seasonal moving average term and one seasonal autoregressive term. 



```{r Seasonally-adjusted model development, warning=FALSE}
ari.fit2 <- co2.log.sa.tr %>% 
  model(ARIMA111101 = ARIMA(season_adjust ~ pdq(1,1,1) + PDQ(1,0,1)),
        ...
        ARIMA121201 = ARIMA(season_adjust ~ pdq(1,2,1) + PDQ(2,0,1)),
        ARIMA122202 = ARIMA(season_adjust ~ pdq(1,2,2) + PDQ(2,0,2)),
        ARIMA013001 = ARIMA(season_adjust ~ pdq(0,1,3) + PDQ(0,0,1)),
        ARIMA022200 = ARIMA(season_adjust ~ pdq(0,2,2) + PDQ(2,0,0)),
...
```{r Seasonally-adjusted model evaluation, warning = FALSE}
ari.fit2 %>% select(ARIMA121201) %>% gg_tsresiduals(lag = 36)
augment(ari.fit2) %>%
  features(.innov, ljung_box, lag=200, dof=4)


bind_rows(
  ari.fit2 %>% select(ARIMA121201) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.sa.te),
  ...
  ari.fit2 %>% select(ARIMA022200) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.sa.te),
...
```{r Non-seasonally model prediction}
co2.nlog.nsa.tr <- co2.log.nsa.tr %>% mutate(
  log_value = exp(log_value) 
)
ari.fit3 %>% forecast(h = 1213) %>% filter(.model == "ARIMA011011") %>% mutate(
  log_value = exp(log_value),
  .mean = exp(.mean)
) %>%
  autoplot(co2.nlog.nsa.tr) + labs(x = "Time", y = "CO2 Level in Atmosphere (ppm)", title = "CO2 Level in the Atmosphere Based on ARIMA Model")

fx2122.90hilo.ari <- ari.fit3 %>% forecast(h = 1213) %>% filter(.model == 'ARIMA011011') %>% hilo(90) %>% unpack_hilo("90%") 

fx2122.90hilo.ari <- fx2122.90hilo.ari %>% mutate(
  .mean = exp(.mean), 
  `90%_lower` = exp(`90%_lower`),
  `90%_upper` = exp(`90%_upper`)
)
fx2122.1st420.ari <- fx2122.90hilo.ari %>% filter(`90%_upper` >= 420) %>% filter(row_number() == 1) %>% select(index) 
fx2122.lst420.ari <- fx2122.90hilo.ari %>% filter(`90%_lower` < 421) %>% filter(row_number() == n()) %>% select(index) 
fx2122.avg420.ari1 <- fx2122.90hilo.ari %>% filter(.mean >= 420) %>% filter(row_number() == 1) %>% select(index) 
fx2122.avg420.ari2 <- fx2122.90hilo.ari %>% filter(.mean < 421) %>% filter(row_number() == n()) %>% select(index) 

fx2122.1st500.ari <- fx2122.90hilo.ari %>% filter(`90%_upper` >= 500) %>% filter(row_number() == 1) %>% select(index) 
fx2122.lst500.ari <- fx2122.90hilo.ari %>% filter(`90%_lower` < 501) %>% filter(row_number() == n()) %>% select(index) 
fx2122.avg500.ari1 <- fx2122.90hilo.ari %>% filter(.mean >= 500) %>% filter(row_number() == 1) %>% select(index) 
fx2122.avg500.ari2 <- fx2122.90hilo.ari %>% filter(.mean < 501) %>% filter(row_number() == n()) %>% select(index) 
```
