---
title: 'Global $CO_{2}$ Emissions'
month: "`r format(Sys.Date(), '%m')`"
year: "`r format(Sys.Date(), '%Y')`"
author: "Edgar Leon, Long Vu, Cameron Yenche"
acknowledgements: "The authors would like to thank their instructors from MIDS 271."
geometry: margin=1in
output:
...
```
The first plot shows the ACF and PACF plots of the raw data after its log transformation. The ACF portion of this plot shows a strong trend coupling with a seasonality signal.
...
```{r present data pipeline and processing, echo = FALSE, warning = FALSE}
url <- "https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_weekly_mlo.csv" 
co2_present <- read_csv(url, skip = 35, show_col_types = FALSE)
...
```
The observed CO2 values indicate that the CO2 level reached 420 ppm the first time in `r obs1st420`. This is earlier than the prediction from the linear model and later than the prediction from the ARIMA model. The linear model (with a tight CI band) predicted that the CO2 level would reach 420 ppm as early as in `r fx2100.1st420.lm`, while the ARIMA model predicted that the CO2 level would reach 420 ppm as early as in `r fx2100.1st420.ari`. Based on their prediction, the ARIMA model provided a more accurate expectation than the linear model. 
...
```{r Seasonally adjusted models, echo = FALSE, warning = FALSE}
...
bind_rows(
  ...
  ari.fit3 %>% select(ARIMA111111) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA111211) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te), 
  ari.fit3 %>% select(ARIMA211211) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA112112) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA112111) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA111112) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  ari.fit3 %>% select(ARIMA013011) %>% forecast(h = 24) %>% fabletools::accuracy(co2.log.nsa.te),
  lm.fx.te2 %>% fabletools::accuracy(co2.log.nsa.te)
)
```
Then, the team developed the models using the non-seasonally adjusted data. The results show that all models produced a similar level of AIC score and out-of-sample MAPE score. We selected the ARIMA model with a seasonal and non-seasonal ma term with one seasonal and non-seasonal differencing since this is a simpler model that performs reasonably well. Interestingly, we also found that the linear model with the updated data does not perform as well as the ARIMA model. 

## Future Predictions

```{r Non-seasonally model prediction, echo = FALSE, warning = FALSE}
...
Our prior, 1997 models demonstrated divergence from modern conditions, revealing overforecasting from 2000 to 2020. In the realm of ARIMA models, we observed CO2 levels were near the upper bounds of the 80% confidence interval, hinting at an increase in the underlying contributor sources to atmospheric CO2. The ARIMA forecast consistently projected lower values yet, maintained a proximity to its upper confidence interval.
...
```{r co2 1997 transformation, echo = FALSE, warning = FALSE}
